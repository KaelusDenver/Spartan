...
// JJB Patch:  replace |h1-h6| in hash below with |h1|h2|h3|h4|h5|h6|
...
// JJB Patch: IE problems with input checkbox and radio controls; see http://helephant.com/2004/10/dom-created-radio-buttons-dont-work-in-ie6/
// Original:  el = (ie && arg1 === 'input') ? '<input type="' + arg2.type + '" />': arg1; //Fix input for ie. REQUIRES attribute type.
// note that for IE8 we are removing the type, Type, and name (if 'radio') properties after use...
          if (ie && arg1 === 'input') {
            if (!arg2.Type) arg2.Type = (arg2.type)? arg2.type : 'text';
            el = '<input type=\"' + arg2.Type + '\"';
            if (arg2.Type.toLowerCase() === 'radio') {
              el = el + ' name=\"' + arg2.name + '\"';
              if (arg2.name) delete arg2.name;
            }
            if (arg2.Type) delete arg2.Type;
            if (arg2.type) delete arg2.type;
            el = el + ' />';
          } else { el = arg1; }
// JJB Patch End
            ...
            for (attr in arg2) { $.attr(el, attr, arg2[attr]); }
				...
				} else if (arg1.match(/(<\S[^><]*>)|(&.+;)/g) !== null && arg1.tagName &&
...
// JJB Patch: IE problems with input checkbox and radio controls; see http://helephant.com/2004/10/dom-created-radio-buttons-dont-work-in-ie6/
// Original: el = (ie && arg[0] === 'input') ? '<input type="' + arg[1].type + '" />': arg[0]; //Fix input for ie. REQUIRES attribute type.
          if (ie && arg[0] === 'input') {
            if (!arg[1].Type) arg[1].Type = (arg[1].type)? arg[1].type : 'text';
            el = '<input type=\"' + arg[1].Type + '\"';
            if (arg[1].Type.toLowerCase() === 'radio') {
              el = el + ' name=\"' + arg[1].name + '\"';
              if (arg[1].name) delete arg[1].name;
            }
            if (arg[1].Type) delete arg[1].Type;
            if (arg[1].type) delete arg[1].type;
            el = el + ' />';
          } else { el = arg[0]; }
// JJB Patch End
				...
				} else if (arg[0].match(/(<\S[^><]*>)|(&.+;)/g) !== null && arg[0].tagName &&
