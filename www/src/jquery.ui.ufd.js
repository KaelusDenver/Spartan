...
$.widget("ui.ufd", {
  ...
  // JJB update: Fixed problem where input val was not reset
  //             if matching index did not change (for
  //             submitFreeText = false); Closest match is
  //             set if no match found (even if index did
  //             not change).
	tryToSetMaster: function() {
      ...
      // JJB:  the selected index may be correct, but the
      // input val may not reflect the selection
      if (!this.options.submitFreeText) this.input.val(option.text);
    ...
    // JJB - don't set the value if submitFreeText
		if (!this.options.submitFreeText) this.input.val(option.text);
	...
	_populateFromMaster: function() {
		...
		if(this.options.triggerSelected){
			...
			this.input.val(this.options.initialFreeText);
		}
...
$.extend($.ui.ufd, {
	...
	defaults: {
		...
		initialFreeText: "", // if not triggerSelected then this value used to set initial value
